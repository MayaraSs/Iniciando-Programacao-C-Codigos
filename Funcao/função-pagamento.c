/*2. Faça um programa em "C" que solicita o total gasto em uma loja,
imprime as opções de pagamento abaixo e
imprime o resultado de acordo com a seleção do usuário.
...
OBS: Fazer uma função que imprime as opções solicita a opção desejada e
 retorna a opção escolhida. No programa principal, t
 estar a opção escolhida e ativar a função correspondente (uma função para cada opção).*/
...
int menu(){
	...
	printf("3. Parcelado (3 a 10) + 5%% juros ao mês\n");
